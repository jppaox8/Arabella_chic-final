...
function ContactCard(){
  ...
  const submit = async (ev)=>{
    ...
    setStatus('sending');
    try{
      const webhookUrl = 'https://hook.us2.make.com/vnccle9u0ur11dij3xohyf1fu5inwkak';
      console.log('Enviando datos de contacto al webhook:', webhookUrl, form);
  ...
  return (
    <div className="rc-wrapper max-w-5xl mx-auto p-6">
      <style>{`
        ...
        @media(max-width:900px){.rc-wrapper{flex-direction:column;min-height:auto}.rc-side{width:100%}.rc-form{min-height:auto}}
      `}</style>
...
// Montaje en #contacto — robusto para distintos timing de carga
function mountContact(){
  try{
    ...
    // si ya contiene contenido, evitamos re-render accidental
    if (el.dataset.mounted) return true;

    // Intenta crear root (React 18)
    try{
      ReactDOM.createRoot(el).render(React.createElement(ContactCard));
    }catch(err){
      // Fallback para entornos extraños
...
// Ejecutar inmediatamente (por si DOMContentLoaded ya ocurrió) y también en el evento
if (!mountContact()){
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', mountContact);
  } else {
    // intenta de nuevo después de un tick
    setTimeout(mountContact, 50);
  }
}

// Captura errores globales para mostrar fallback y evitar página en blanco
